...
std::vector<std::vector<Matrix<long double>>> clip_singular_1d(std::vector<std::vector<Matrix<long double>>> kernels,
                                                               const size_t signal_size, const long double lower_bound,
    ...
    auto values = svd_convolution_1d(kernels, signal_size, &left_basis, &right_basis, false, eps);
    ...
    auto matrix_kernels =
        left_basis *
        Matrix<long double>::diagonal(values, C_out * (signal_size - kernel_size + 1), C_in * signal_size) *
        right_basis;
